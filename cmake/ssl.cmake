...
MACRO(MYSQL_CHECK_SSL_DLLS)
  IF (WITH_SSL_PATH AND (APPLE OR WIN32 OR LINUX_STANDALONE OR LINUX_RPM))
    ...
    IF(LINUX)
      ...
      ELSEIF(CRYPTO_EXT STREQUAL ".a" AND OPENSSL_EXT STREQUAL ".a")
        MESSAGE(STATUS "Use static ssl libraries")
    ...
    IF(WIN32)
      ...
      FIND_FILE(HAVE_CRYPTO_LIB
          NAMES
          "${CRYPTO_NAME}.lib"
          PATHS "${WITH_SSL_PATH}/lib"
          NO_DEFAULT_PATH
        )
      FIND_FILE(HAVE_OPENSSL_LIB
          NAMES
          "${OPENSSL_NAME}.lib"
          PATHS "${WITH_SSL_PATH}/lib"
          NO_DEFAULT_PATH
      ...
      IF(HAVE_CRYPTO_DLL AND HAVE_OPENSSL_DLL)
        ...
        COPY_OPENSSL_BINARY(${OPENSSL_EXECUTABLE} "" "" openssl_exe_target)
        ADD_DEPENDENCIES(${openssl_exe_target} copy_openssl_dlls)
      ELSEIF(HAVE_CRYPTO_LIB AND HAVE_OPENSSL_LIB)
        MESSAGE(STATUS "Use static ssl libraries")
        SET(COPIED_OPENSSL_LIBRARY "${OPENSSL_LIBRARY}")
        SET(COPIED_CRYPTO_LIBRARY "${CRYPTO_LIBRARY}")
      ELSE()
